...
COMPONENT_INIT
{
    LE_INFO("secure storage independence test, appB");
    ...
    if (le_arg_NumArgs() == 1)
    {
        ...
        if (strcmp(option, "read") == 0)
        {
            ...
            // after appA has finished writing a different secure storage content with appB's
            // secure storage item name, if appB reads the same content as it wrote, mark this
            // TC passed; failed otherwise.
            if (strcmp("123String", outBuffer) == 0)
            {
                LE_INFO("appB successfully read its own written content");
            }
        ...
        else if (strcmp(option, "write") == 0)
        {
            ...
            le_result_t result = le_secStore_Write("file1", (uint8_t*)"123String", 10);
