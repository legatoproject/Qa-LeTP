  ...
  * @verbatim
  * $ app start flashApp
  * $ app runProc flashApp --exe=flashApp -- <arg1> [<arg2>]
  *
  * Example:
  * $ app runProc flashApp --exe=flashApp -- help
  * @endverbatim
  *
...
typedef struct
{
    ...
    le_result_t (*flashApp)(char **args); ///< The test function to call
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_Help(char **args);
static le_result_t Flash_Info(char **args);
static le_result_t Flash_Dump(char **args);
static le_result_t Flash_Flash(char **args);
static le_result_t Flash_FlashErase(char **args);
static le_result_t Flash_Copy(char **args);
static le_result_t Flash_InfoUbi(char **args);
static le_result_t Flash_DumpUbi(char **args);
static le_result_t Flash_FlashUbi(char **args);
static le_result_t Flash_CreateUbi(char **args);
static le_result_t Flash_CreateUbiVol(char **args);
static le_result_t Flash_DeleteUbiVol(char **args);
static le_result_t Flash_CopyUbi(char **args);
...
static FlashApp_t FlashApp[] =
{
    ...
    { "flash-erase",    2, Flash_FlashErase,
...
)
{
    ...
    Print("Usage of the 'flashApp' application is:", NULL);
    for(idx = 0; idx < NUM_ARRAY_MEMBERS(FlashApp); idx++)
    {
        Print("flashApp -- %s", FlashApp[idx].usage);
    }
    return LE_FAULT;
}
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_Flash
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_FlashErase
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_InfoUbi
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_DumpUbi
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_FlashUbi
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_CreateUbi
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_CreateUbiVol
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_DeleteUbiVol
(
    char **args
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_result_t Flash_CopyUbi
(
    char **args
)
{
...
COMPONENT_INIT
{
    ...
    if (le_arg_NumArgs() >= 1)
    {
        ...
        for(idx = 0; idx < NUM_ARRAY_MEMBERS(FlashApp); idx++)
        {
            if((0 == strcmp(actionStr, FlashApp[idx].action)) &&
               (le_arg_NumArgs() >= (FlashApp[idx].nbArg + 1)))
            {
               for(iarg = 0; iarg < FlashApp[idx].nbArg; iarg++)
               {
               ...
               if (LE_OK == res)
               {
                   res = FlashApp[idx].flashApp((char **)args);
               }
               else
               {
    ...
    LE_DEBUG ("flashApp: unsupported action '%s'", actionStr);
    Flash_Help(NULL);
    exit(EXIT_FAILURE);
}
