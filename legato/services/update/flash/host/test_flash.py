"""@package swUpdateFlashModule The flash APIs test.
...
PARTITIONS_MTDLIST = [["tz_update", "tz_active", "tz_1", "tz_2"],
                      ...
                      ["system_update", "system_active", "system_1",
                       "system_2"],
                      ["lefwkro_update", "lefwkro_active", "lefwkro_1",
                      "lefwkro_2"]]

PARTITIONS_UBILIST = [["system", "rootfs"],
                      ["system", "rootfs_hs"],
                      ["system", "rootfs_rhs"],
                      ["system", "rootfs_srhs"],
                      ["system", "rootfs_cert"],
...
# Test functions
# ==================================================================================================
def L_SwUpdate_Flash_MtdInfo_0001(target, legato, app_leg):
    """
    Getting info from partition: tz,rpm about bootsystem lefwkro customer.
...
def L_SwUpdate_Flash_UbiInfo_0001(target, legato, app_leg):
    """
    Getting information from Ubi volume system/system 2.
    ...
    for i in range(len(PARTITIONS_UBILIST)):
        ...
        ext, rsp = target.run(cmd, withexitstatus=1)
        pattern = "flashApp* has exited with exit code 0"
        error_msg = ("[FAILED] Getting info from" +
                     PARTITIONS_UBILIST[i][0] + ", " +
...
def L_SwUpdate_Flash_MtdInfo_0009(target, legato, app_leg):
    """
    Getting info from partition: tz,rpm aboot boot system lefwkro customer.
    ...
    pattern = ("Bad Block ([0-9]*), Block ([0-9]*), " +
               "Erase Block Size ([0-9]*), Page Size ([0-9]*)")
    partition_info = legato.get_from_target_log(pattern)
    if partition_info:
        swilog.info("partition Info are Bad Block: %s" %
                    partition_info.group(1))
        swilog.info("partition Info are Block: %s" %
                    partition_info.group(2))
        swilog.info("partition Info are Erase Block Size: %s" %
                    partition_info.group(3))
        swilog.info("partition Info are Page Size:  %s " %
                    partition_info.group(4))
    ...
    legato.clear_target_log()
    time.sleep(5)
    target.run("app runProc flashApp flashApp -- info lefwkro2",
               withexitstatus=1)
    pattern = "flashApp* has exited with exit code 0"
    ...
    legato.clear_target_log()
    time.sleep(5)
    target.run("app runProc flashApp flashApp -- info lefwkro_2",
               withexitstatus=1)
    pattern = "flashApp* has exited with exit code 0"
