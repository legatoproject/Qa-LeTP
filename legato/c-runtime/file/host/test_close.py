...
TEST_TOOLS = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                          'tools')
APP_NAME = "atomClose"
APP_PATH = os.path.join(os.path.join(TEST_RESOURCES, "atomClose"),
                        "atomClose.adef")
...
# Test functions
# ==================================================================================================
def L_AtomicFile_Operation_0029(target, legato, app_leg, init_atomicFile):
    """
    ...
    test_descriptions = ["openWrtFlockOK", "tryOpenWrtFlockOK",
                         "createWrtFlockOK", "tryCreateWrtFlockOK"]
    ...
    for td in test_descriptions:
        ...
        if legato.ssh_to_target(
                r" cat %s | grep -q \"String Foo\"" % (test_file_path)) != 0:
                assert 0, "[FAILED] le_atomFile_Close doesn't"\
...
def L_AtomicFile_Operation_0030(target, legato, app_leg, init_atomicFile):
    """
    Purpose: Verify that le_atomFile_Close commits all changes,
            closes the file descriptor
    ...
    hw_file_path = os.path.join(TEST_TOOLS, "testFile.txt")
    test_file_path = init_atomicFile
    test_descriptions = ["openReadFlockOK", "tryOpenReadFlockOK",
                         "createReadFlockOK", "tryCreateReadFlockOK"]
