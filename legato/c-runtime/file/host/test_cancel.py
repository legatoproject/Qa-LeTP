...
TEST_TOOLS = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                          'tools')
APP_NAME = "atomCancel"
APP_PATH = os.path.join(os.path.join(TEST_RESOURCES, "atomCancel"),
                        "atomCancel.adef")
...
# Test functions
# ==================================================================================================
def L_AtomicFile_Operation_0031(target, legato, app_leg, init_atomicFile):
    """
    ...
    for td in test_descriptions:
        ...
        legato.runProc(test_app_name, test_app_proc_name,
                       test_file_path, td)
        assert legato.wait_for_log_msg("le_atomFile_Cancel is called",
                                       20) is True
...
def L_AtomicFile_Operation_0032(target, legato, app_leg, init_atomicFile):
    """
    Purpose: Verify that le_atomFile_Cancel cancels all changes
            and closes the file descriptor for
