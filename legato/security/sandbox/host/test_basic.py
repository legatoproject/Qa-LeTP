...
def check_message_in_log(logread, msg_1, msg_2):
    ...
    """
    assert logread.expect([pexpect.TIMEOUT, msg_1], 30) == 1, "[FAILED] Cannot \
find out: %s" % msg_1
    assert logread.expect([pexpect.TIMEOUT, msg_2], 30) == 1, "[FAILED] Cannot \
find out: %s" % msg_2
...
# Test functions
# ==================================================================================================
def L_SandBox_0001(target, legato, logread, app_leg):
    """
    Sandbox app is sandboxed
    1) Run SB_10_20_30() from the sandbox app "sandbox.c"
    2) Verify if correct home directory has been
    set for sandbox app (/home/sandbox)
    ...
    # TODO: since home dir is no longer "/home/appAPPNAME",
...
def L_SandBox_0005(target, legato, logread, app_leg):
    """
    BreakOut1
    ...
    assert logread.expect([pexpect.TIMEOUT, create_dir_msg_fail], 5) == 0, "[FAILED] \
Find out: %s" % create_dir_msg_fail


def L_SandBox_0006(target, legato, logread, app_leg):
    """
    BreakOut2
...
def L_SandBox_0004(target, legato, logread, app_leg):
    """
    ...
    cur_rsp = legato.ssh_to_target("/sbin/logread | grep \\\"%s\\\""
                                   % cur_pid_msg, output=True)
    parent_rsp = legato.ssh_to_target("/sbin/logread | grep \\\"%s\\\""
                                      % parrent_pid_msg, output=True)
    ...
    kill_parent_msg = ">> trying to kill pid \\[%s\\], error "\
