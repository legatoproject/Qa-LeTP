...
APP_PATH_L_SampleApps_Sms_0001 = "%s/apps/sample/%s/" % (
    LEGATO_ROOT,
    APP_NAME_L_SampleApps_Sms_0001,
)
...
def L_SampleApps_Sms_0001(target, legato, logread, installapp_cleanup):
    ...
    exit_status, rsp = target.run(cmd, timeout=100, withexitstatus=True)
    swilog.debug(rsp)
    assert exit_status == 0, "[FAILED] Send test sms"
    ...
    check_log(logread, "the message has been successfully deleted from storage.")
