...
# Test functions
# ==================================================================================================
def L_SampleApps_Karaoke_0001(target, legato, installapp_cleanup):
    """
    This script will
    ...
    cmd = "/legato/systems/current/bin/app runProc %s selector -- fast\r"\
          % APP_NAME_L_SampleApps_Karaoke_0001

    target.send(cmd)
    assert target.expect_in_order(["Danny Boy", "Jingle Bells",
                                   "Deck The Halls"], timeout=20)
    ...
    cmd = "1\n"
    target.send(cmd)
    assert target.expect([pexpect.TIMEOUT, "'Danny Boy' selected.  "
                                           "Playing now."], 20)

    cmd = "2\n"
    target.send(cmd)
    assert target.expect([pexpect.TIMEOUT, "'Jingle Bells' selected.  "
                                           "Playing now."], 20)

    cmd = "3\n"
    target.send(cmd)
    assert target.expect([pexpect.TIMEOUT, "'Deck The Halls' selected.  "
                                           "Playing now."], 20)
