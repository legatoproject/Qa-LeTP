...
# Functions
# =================================================================================================
def verify_when_build_successful(ipc_type_str,
                                 interface_opts_str,
                                 build_cmd_str,
                                 is_binded_str,
                                 api_type_str):
    ...
    if (ipc_type_str == "client-side IPC"):
        if ((interface_opts_str == "[optional]") or
           (interface_opts_str == "[optional] [manual-start]")):
            ...
            swilog.info("%s doesn't complain when the %s API is %s to %s "
                        ...
                        % (build_cmd_str, ipc_type_str, is_binded_str,
                           api_type_str, interface_opts_str))
        else:
            # Build successful when client-side IPC
            # With interface options [optional] [types-only]
            swilog.error("%s doesn't complain when the %s API is %s to "
                         "with the interface options: %s"
                         % (build_cmd_str, ipc_type_str, is_binded_str,
                            api_type_str, interface_opts_str))
    else:
        ...
        swilog.error("%s doesn't complain when the %s provides %s "
                     "with interface options: %s"
                     % (build_cmd_str, ipc_type_str,
                        api_type_str, interface_opts_str))


def verify_when_build_fail(ipc_type_str,
                           interface_opts_str,
                           build_cmd_str,
                           is_binded_str,
                           api_type_str):
    ...
    if (ipc_type_str == "client-side IPC"):
        if(interface_opts_str == "[optional] [types-only]"):
            ...
            # With interface options [optional] [types-only]
            swilog.info("%s complains when the %s is %s to %s "
                        "with the interface options: %s"
                        % (build_cmd_str, ipc_type_str, is_binded_str,
                           api_type_str, interface_opts_str))
        else:
            # Build unsuccessful when client-side IPC
            # With interface options [optional]/[optional] [manual-start]
            swilog.error("%s complains when the %s API is %s to %s "
                         "with the interface options: %s"
                         % (build_cmd_str, ipc_type_str, is_binded_str,
                            api_type_str, interface_opts_str))
    else:
        ...
        # With interface options that contain optional
        swilog.info("%s complains when the %s provides %s "
                    "with interface options: %s"
                    % (build_cmd_str, ipc_type_str, api_type_str,
                       interface_opts_str))


def build_sys(legato,
              api_type_dir_name,
              ipc_type_dir_name,
              build_cmd_dir_name,
              interface_opts,
    ...
    and verify that it's built successfully or not
    ...
    system_file_path = "%s/%s/%s" % (api_type_dir_name,
                                     ipc_type_dir_name,
                                     build_cmd_dir_name)
    ...
    sys_path = "%s/cdef/interfaceOptions/optional/%s"\
               % (TEST_RESOURCES, system_file_path)
    ...
    if build_status is False:
        verify_when_build_successful(ipc_type_dir_name,
                                     interface_opts,
                                     build_cmd_dir_name,
                                     binding_str,
                                     api_type_dir_name)
    else:
        verify_when_build_fail(ipc_type_dir_name,
                               interface_opts,
                               build_cmd_dir_name,
                               binding_str,
                               api_type_dir_name)
...
def build_app(legato,
    ...
    app_files_path = "%s/%s/%s" % (api_type_dir_name,
                                   ipc_type_dir_name,
                                   build_cmd_dir_name)
    adef_file_name = "%s_%s" % (interface_opts, binding_str)

    # The appropriate directory that contains the definition files
    app_path = "%s/cdef/interfaceOptions/optional/%s" % (TEST_RESOURCES,
                                                         app_files_path)
    # Parse args to readable string
    if (api_type_dir_name == "legatoAPI"):
        api_type_dir_name = "legato API"
...
# Test functions
# =================================================================================================
def L_CDEF_0006(legato, init_test):
    """
    Verify that "mkapp" will not complain if there's no optional client-side
    IPC API to be bound with customized server-side interfaces

    Args:
        legato: fixture to call useful functions regarding legato
        init_test: initial the test

    """

    build_app(legato, "customizedAPI", "clientIPC",
              "mkapp", "optional", "withoutBindings")
...
def L_CDEF_0007(legato, init_test):
    """
    Verify that "mkapp" will not complain if there's no optional client-side
    ...
    build_app(legato, "legatoAPI", "clientIPC",
              "mkapp", "optional", "withoutBindings")
...
def L_CDEF_0008(legato, init_test):
    """
...
def L_CDEF_0009(legato, init_test):
    """
    Verify that "mkapp" will be successful if optional client-side IPC API
...
def L_CDEF_0010(legato, init_test):
    """
    ...
    build_app(legato, "legatoAPI", "clientIPC", "mkapp", "optional_typesOnly",
              "withoutBindings", build_status=True)
...
def L_CDEF_0011(legato, init_test):
    """
    ...
    build_app(legato, "legatoAPI", "clientIPC", "mkapp",
              "optional_manualStart", "withoutBindings")
...
def L_CDEF_0012(legato, init_test):
    """
    ...
    build_app(legato, "legatoAPI", "serverIPC", "mkapp", "optional",
              "withoutBindings", build_status=True)
...
def L_CDEF_0013(legato, init_test):
    """
    Verify that "mksys" will not complain if there's no optional client-side
    IPC API to be bound with Legato's server-side interfaces
    ...
    build_sys(legato, "legatoAPI", "clientIPC", "mksys", "optional",
              "withoutBindings")
...
def L_CDEF_0014(legato, init_test):
    """
    Verify that "mksys" will not complain if there's no optional client-side
    IPC API to be bound with customized server-side interfaces
...
def L_CDEF_0015(legato, init_test):
    """
    Verify that "mksys" will be successful if optional client-side IPC API is
    bound with Legato's server-side interfaces
...
def L_CDEF_0016(legato, init_test):
    """
    Verify that "mksys" will be successful if optional client-side IPC API is
    bound with customized server-side interfaces
...
def L_CDEF_0017(legato, init_test):
    """
    ...
    build_sys(legato, "legatoAPI", "clientIPC", "mksys", "optional_typesOnly",
              "withoutBindings", build_status=True)
...
def L_CDEF_0018(legato, init_test):
    """
    Verify that "mksys" will be successful if client-side IPC API options are
    optional and manual start and to require Legato API
    ...
    build_sys(legato, "legatoAPI", "clientIPC", "mksys",
              "optional_manualStart", "withoutBindings")
...
def L_CDEF_0019(legato, init_test):
    """
    Verify that "mksys" will complain "[optional]" is only for client-side IPC
    option when the server app provides Legato API
    ...
    build_sys(legato, "legatoAPI", "serverIPC", "mksys", "optional",
              "withoutBindings", build_status=True)
...
def L_CDEF_0020(legato, init_test):
    """
    ...
    build_app(legato, "legatoAPI", "clientIPC", "mkapp", "optional_typesOnly",
              "withBindings", build_status=True)
...
def L_CDEF_0021(legato, init_test):
    """
    Verify that requires section can't have duplicated API
    ...
    build_app(legato, "customizedAPI", "clientIPC", "mkapp",
              "optional_typesOnly", "withBindings", build_status=True)
...
def L_CDEF_0022(legato, init_test):
    """
    Verify that "mkapp" will be successful if client-side IPC API options are
    optional and manual start and to require customized API
...
def L_CDEF_0023(legato, init_test):
    """
    Verify that "mkapp" will be unsuccessful if client-side IPC API options
    ...
    build_app(legato, "customizedAPI", "clientIPC", "mkapp",
              "optional_typesOnly", "withoutBindings", build_status=True)
...
def L_CDEF_0024(legato, init_test):
    """
    Verify that "mksys" will be successful if client-side IPC API options are
    optional and manual start and to require customized API
    ...
    build_sys(legato, "customizedAPI", "clientIPC", "mksys",
              "optional_manualStart", "withoutBindings")
...
def L_CDEF_0025(legato, init_test):
    """
    Verify that "mksys" will be unsuccessful if client-side IPC API options
    are types-only and optional and to require customized API
...
def L_CDEF_0026(legato, init_test):
    """
    Verify that "mkapp" will complain "[optional]" is only for client-side IPC
    option when the server app provides customized API
    ...
    build_app(legato, "customizedAPI", "serverIPC", "mkapp", "optional",
              "withoutBindings", build_status=True)
...
def L_CDEF_0027(legato, init_test):
    """
    Verify that "mksys" will complain "[optional]" is only for client-side IPC
    option when the server app provides customized API
    ...
    build_sys(legato, "customizedAPI", "serverIPC", "mksys", "optional",
              "withoutBindings", build_status=True)
